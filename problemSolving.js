...
class Matrix {
  ...
  get columns() {
    return Array.from(this.rows[0], (_, n) => this.rows.map((row) => row[n]));
  }
...
List.prototype.compare = function (listTwo) {
  if (this.isSublist(listTwo)) {
    if (this.values.length == listTwo.values.length) {
      return "EQUAL";
    }
    return "SUBLIST";
  } else if (listTwo.isSublist(this)) {
    return "SUPERLIST";
  }
  return "UNEQUAL";
};
